...
class DropBoxController {
  ...
  connectFireBase() {
    ...
    var firebaseConfig = {
      apiKey: "AIzaSyBG1MgJya489DXvkEmgj2NTwoA6fQCia0Y",
      authDomain: "dropboxclone-90e0e.firebaseapp.com",
      databaseURL: "https://dropboxclone-90e0e-default-rtdb.firebaseio.com",
      projectId: "dropboxclone-90e0e",
      storageBucket: "dropboxclone-90e0e.appspot.com",
      messagingSenderId: "1039468894333",
      appId: "1:1039468894333:web:5ffc3570ced3b8a7bcf914",
      measurementId: "G-SVP63GL7ZL",
    };
    ...
    this.getSelection().forEach((li) => {
      ...
      promises.push(
        new Promise((resolve, reject) => {
          let refFile = this.currentFolder.join("/");

          let fileName = file.name;

          if (file.type === "folder") {
            this.removeFolderTask(refFile, fileName)
              .then(() => {
                resolve({
                  fields: {
                    key,
            ...
            this.removeFile(refFile, fileName)
              .then(() => {
                resolve({
                  fields: {
                    key,
    ...
    return new Promise((resolve, reject) => {
      ...
      folderRef.on("value", (snapshot) => {
        snapshot.forEach((item) => {
          ...
          if (data.type == "folder") {
            this.removeFolderTask(ref + "/" + name)
              .then(() => {
                resolve({
                  fields: {
                    key: data.key,
  ...
  initEventsLi(li) {
    ...
    li.addEventListener("click", (e) => {
      if (e.shiftKey) {
        ...
        if (firstLi) {
          let ulLis = li.parentElement.childNodes;
          let FileIndexStart;
          let FileIndexEnd;

          ulLis.forEach((el, index) => {
            if (firstLi === el) FileIndexStart = index;
            if (li === el) FileIndexEnd = index;
          });

          let FileIndexSelected = [FileIndexStart, FileIndexEnd].sort();

          ulLis.forEach((el, i) => {
            if (i >= FileIndexSelected[0] && i <= FileIndexSelected[1]) {
          ...
          // console.log(li);
          // console.log(ulLis);
          // console.log(FileIndexSelected, "selected");
        }
      }
